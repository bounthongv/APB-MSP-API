...
def cancel_expense():
    ...
    try:
        ...
        # Only allow cancellation for 'wait' or 'success' statuses
        if expense_record.status not in ['wait', 'success']:
            return jsonify({"error": f"Expense with exp_no '{exp_no}' cannot be cancelled. Only expenses with status 'wait' or 'success' can be cancelled."}), 400
